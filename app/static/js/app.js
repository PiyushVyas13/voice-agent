...
// Floating widget and modal logic
...
function openChatModal() {
  ...
  // Start voice mode automatically if not already started
  if (!is_audio) {
    is_audio = true;
    startAudio();
  ...
  // Send greeting if not already greeted this session
  if (!greetedThisSession) {
    greetedThisSession = true;
    // Wait for websocket to be open before sending greeting
    const tryGreet = () => {
      if (websocket && websocket.readyState === WebSocket.OPEN) {
        sendMessage({
          mime_type: "text/plain",
          data: "Hey, how can I assist you today?",
          role: "user",
        });
      } else {
        setTimeout(tryGreet, 100);
      }
    };
    tryGreet();
  }
...
supportWidget.addEventListener("click", openChatModal);
...
endCallBtn.addEventListener("click", function() {
  ...
  endMsg.textContent = "Call ended.";
  endMsg.className = "agent-message";
  messagesDiv.appendChild(endMsg);
  messagesDiv.scrollTop = messagesDiv.scrollHeight;
});

// Hide old status bar if present
...
// Helper to stop bot audio playback
...
function addSubmitHandler() {
  messageForm.onsubmit = function (e) {
    ...
    if (message) {
      // Stop bot audio if user interrupts
      stopBotAudioPlayback();
